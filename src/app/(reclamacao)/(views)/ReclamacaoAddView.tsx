...
import { CreateReclamacaoDTO, ReclamacaoSchema } from '@/app/(reclamacao)/(model)/(schema)/reclamacao.schema'
...
export default function ReclamacaoAddView() {
  ...
  const locationToggle = useRef<boolean>(true)
  const cssStyle = 'w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 resize-vertical transition-colors'


  const form = useForm<z.infer<typeof ReclamacaoSchema>>({
  ...
  const onSubmit: SubmitHandler<z.infer<typeof ReclamacaoSchema>> = async (data) => {
      if (!data) return
      try {
        ...
        const payload: CreateReclamacaoDTO = {
          // form data doesn't include 'situacao' (omitted in ReclamacaoSchema), provide default
          situacao: 'aberta',
          ...data,
        }
        const reclamacao = await ReclamacaoHttp.create(payload)
  ...
  // Extrair setValue para usar em callbacks memoizados
  const { setValue } = form
  const handleLocationSelect = useCallback(async (lat: number, lng: number) => {
    try {
      ...
      // // montar um endereço simples
      const road = addr.road || addr.pedestrian || addr.cycleway || ''
      const houseNumber = addr.house_number || ''
      const endereco = [road, houseNumber].filter(Boolean).join(' ').trim()

      setValue('endereco', endereco || data.display_name || '')
      setValue('bairro', addr.suburb || addr.neighbourhood || addr.village || '')
      setValue('cidade', addr.city || addr.town || addr.village || addr.county || '')
      setValue('estado', addr.state || '')
      locationToggle.current = false
      if (addr.postcode) setValue('cep', addr.postcode)
      ...
      async function fetchAddress() {
        ...
        if (findCandidates.ok) {
          ...
          if (
            ...
            locationToggle.current
          ) {
            const location = candidatesData.candidates[0].location
            form.setValue('latitude', location.y)
            form.setValue('longitude', location.x)
            mapRef.current?.setMarker(location.y, location.x)
            setMapPosition({ lat: location.y, lng: location.x })
          }
        }
        locationToggle.current = true
      }
      // deixe so os numeros
      if (!cepWatch) return
