...
import { updateReclamacaoDTO, UpdateReclamacaoDTO } from '@/app/(reclamacao)/(model)/(schema)/reclamacao.schema'
...
export function FormComponent(props: { editingId?: string; emit: (data: any) => void }) {
  const { editingId, emit } = props
  ...
  const form = useForm<UpdateReclamacaoDTO>({
    resolver: zodResolver(updateReclamacaoDTO),
    defaultValues: {
      descricao: '',
      cep: '',
      endereco: '',
      ...
      estado: '',
      numero: '',
      latitude: -12.963308366330674, // Salvador, BA como padrão
      longitude: -38.500841214385076,
      usuarioId: '' // Será preenchido pelo hook
    }
  })

  const onSubmit = async (data: UpdateReclamacaoDTO) => {
    if (!data) return
    try {
      ...
      const reclamacao = await ReclamacaoHttp.update(editingId!, data)
      console.log('Reclamação criada:', reclamacao)
      await imagemReclamacaoHttp.create(editingId!, selectedImages)

      notify({message: 'Reclamação salva com sucesso!', type: 'success' })
  ...
  // If editingId is provided, fetch existing reclamacao and prefill form
  useEffect(() => {
    if (!editingId) return
    ;(async () => {
      try {
        const data = await ReclamacaoHttp.findById(editingId) as any
        form.reset(data)
        const imagens =  await imagemReclamacaoHttp.findByReclamacaoId(editingId)
        const previewUrls = imagens.map((img: any) => {
          const blob = new Blob([new Uint8Array(img.arquivo.data)], { type: img.tipoMime })
          return URL.createObjectURL(blob)
        })
        setImagePreviews(previewUrls)
        setSelectedImages(imagens.map((img: any) => new File([new Uint8Array(img.arquivo.data)], img.nomeArquivo, { type: img.tipoMime })))

        if (['encaminhada', 'resolvido'].includes(data.situacao)) {
          setIsReadOnly(true)
        }
      } catch (err) {
        console.error('Erro ao carregar reclamação para edição', err)
        notify({ message: 'Erro ao carregar reclamação para edição', type: 'error' })
      }
    })()
  }, [editingId, form])
      ...
      <div className="flex-1 overflow-auto">
        <form className="p-6 space-y-4" onSubmit={form.handleSubmit(onSubmit)}>
          ...
          <div className="bg-gray-50 p-4 rounded-lg border-2 border-dashed border-gray-300">
            ...
            <div className="mt-4 pt-4 border-t border-gray-200">
              ...
              <div className="space-y-4">
                ...
                <div hidden={isReadOnly} className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-emerald-400 transition-colors">
                  <input
                    ref={fileInputRef}
                    type="file"
                    accept="image/*"
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-4">
            <button
              type="submit"
              className={`flex-1 py-3 px-6 rounded-lg transition-colors font-semibold flex items-center justify-center ${
                isReadOnly
